...
def first_pass( commands ):
    ...
    if b_vary and not b_frame:
        print 'error: vary without frame'
        exit(1)
    if b_frame and not b_basename:
        basename = 'x'
        print 'setting basename to x since no name was found'
...
def second_pass( commands, num_frames ):
    ...
    for y in commands:
        if y['op'] == 'vary':
            knob = y['knob']
            start_frame = y['args'][0]
            start_vary = y['args'][2]
            end_frame = y['args'][1]
            end_vary = y['args'][3]

            if(start_frame < 0 or end_frame >= num_frames or end_frame <= start_frame):
                print("vary is incorrect")
                exit(1)

            i = (end_vary - start_vary) / (end_frame - start_frame)

            x = int(start_frame)
            while x < end_frame:
                knobs[x][knob] = start_vary + i * (x - start_frame)
                x += 1
    return knobs
